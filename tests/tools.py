...
class PyCKtools:
    ...
    def check_folder(thisfolder):
        ...
        if not os.path.exists(thisfolder):
            print(
            f"Error: folder {thisfolder} does not exist."
            )
            return 1
        return 0
    ...
    def get_tolerances(
        ...
        """
        Find the tolerance values according to the vriable type.
        """
    ...
    def compare_list(r_list, b_list, atol: float, rtol: float) -> tuple[int, list, list]:
        """
